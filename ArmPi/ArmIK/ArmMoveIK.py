...
#The manipulator moves according to the angle calculated by inverse kinematics
ik = IK('arm')
#Set the link length
l1 = ik.l1 + 0.75
l4 = ik.l4 - 0.15
...
class ArmIK:
    ...
    def transformAngelAdaptArm(self, theta3, theta4, theta5, theta6):
        #Convert the angle calculated by inverse kinematics to the pulse width value corresponding to the servo
    ...
    def setPitchRange(self, coordinate_data, alpha1, alpha2, da = 1):
        #Given the range of coordinates coordinate_data and pitch angle alpha1, alpha2, automatically find the appropriate solution within the range
        ...
        #da is the angle that increases each time when the pitch angle is traversed
        x, y, z = coordinate_data
        if alpha1 >= alpha2:
            da = -da
        for alpha in np.arange(alpha1, alpha2, da):# traverse to solve
    ...
    def setPitchRangeMoving(self, coordinate_data, alpha, alpha1, alpha2, movetime=None):
        #Given the coordinates coordinate_data and pitch angle alpha, and the range of pitch angle range alpha1, alpha2, automatically find the solution closest to the given pitch angle, and go to the target position
